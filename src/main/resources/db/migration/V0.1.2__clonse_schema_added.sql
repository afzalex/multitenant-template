...
BEGIN
  ...
  LOOP
    ...
    FOR column_, default_ IN
      ...
      dest_schema||'.') FROM information_schema.COLUMNS WHERE table_schema = dest_schema AND 
    ...
    FOR trigger_name_, trigger_timing_, trigger_events_, trigger_orientation_, trigger_action_ IN
      ...
      FROM information_schema.TRIGGERS WHERE event_object_schema=source_schema and 
      event_object_table=object GROUP BY trigger_name, action_timing, 
...
-- Create Views
   FOR objetovw IN
        SELECT table_name,replace(view_definition,source_schema||
    ...
    LOOP        
        ...
        EXECUTE 'CREATE OR REPLACE VIEW ' || buffer || 
        ' AS '||objetovw.view_definition;
    END LOOP;

  -- first create Extension in source schema "uuid-ossp" , 
